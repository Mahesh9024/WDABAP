...
public class WD_ABA_P13N_CEF_AUTO_DD 
{
	...
	@Test
	public void WD_WD_ABA_P13N_CEF_AUTO_DD()
	{
...
//•For the line labelled ''DESCRIPTION'  ' 
		Utility.clickOnElementUsingJavaScriptExecutor(driver, "RadioButon.REASON_FOR_STOP.Sichtbar.xpath");
		...
		try {
			Assert.assertEquals(true, Utility.isElementPresentByXpath(driver, "Dropdown.GrfurStop.xpath"));
...
//•Open context menu for the header of the group box called 'Kundenerweiterungsfelder' and choose 'Einstellungen für aktuelle Konfiguration'
		...
		driver.switchTo().defaultContent();
		try {
			Assert.assertEquals(false,Utility.isElementPresentByXpath(driver, "Dropdown.GrfurStop.xpath"));
